...
It is adopted and modified from Hans Dietrich's XMessageBox:
...
//#include "stdafx.h"  // PapyrusPlugin modification -- not used
///////////////////////////////////////////////////////////////////////////////
...
#include "..\npp\NppDarkMode.h"  // PapyrusPlugin modification -- dark mode support
...
// the following line:
//
//#define XMESSAGEBOX_AUTO_VISTA_STYLE  // PapyrusPlugin modification -- do not enforce Vista style
...
protected:
  ...
  {
    UINT resId;
    const TCHAR *pszDefaultText;  // PapyrusPlugin modification -- add "const"
  };
...
static const TCHAR *szAlphabet = _T("ABCDEFGHIJKLMNOPQRSTUVWXYZ")  // PapyrusPlugin modification -- add "const"
               _T("0123456789")
  ...
  m_bNarrow(pXMB->dwOptions & XMSGBOXPARAMS::Narrow)        //+++1.8
{
  ...
  if (lpszMessage)
  {
    if (HIWORD(lpszMessage) == NULL)
    {
      ...
      if (::LoadString(hInstanceStrings,
               ...
               LOWORD(lpszMessage),  // PapyrusPlugin modification -- remove unnecessary cast "DWORD"
               m_lpszMessage,
               MessageSize-1) == 0)
        m_lpszMessage[0] = _T('\0');
    }
  ...
  if (lpszCaption)
  {
    if (HIWORD(lpszCaption) == NULL)
    {
      ...
      if (::LoadString(hInstanceStrings,
               ...
               LOWORD(lpszCaption),  // PapyrusPlugin modification -- remove unnecessary cast "DWORD"
               m_lpszCaption,
               MessageSize-1) == 0)
        m_lpszCaption[0] = _T('\0');
    }
  ...
  for (int button_index = 0; button_index < LAST_BUTTON; button_index++)
  {
    ...
    ::GetTextExtentPoint32(hdc, m_szButtonText[button_index], 
      ...
      static_cast<int>(_tcslen(m_szButtonText[button_index])), &size);  // PapyrusPlugin modification -- cast from size_t to int
  ...
  else
  {
    ...
    while (cp != NULL)
    {
      ...
      ::GetTextExtentPoint32(hdc, cp, static_cast<int>(_tcslen(cp)), &size);  // PapyrusPlugin modification -- cast from size_t to int
                         ...
                         LPARAM lParam)
{
  ...
  switch (message)
  {
    case WM_CTLCOLORSTATIC:    //+++1.8
    {
      ...
      // PapyrusPlugin modification -- dark mode support
    ...
    case WM_PRINTCLIENT:
    {
      if (!Me->m_hBackgroundBrush && NppDarkMode::isEnabled())
      {
        return TRUE;
      }
      return FALSE;
    ...
    {
      //if (Me->m_hBackgroundBrush)
      if (Me->m_hBackgroundBrush || NppDarkMode::isEnabled())  // PapyrusPlugin modification -- dark mode support
      {
        HDC hdc = (HDC) wParam;
        ...
        HBRUSH hBrush = Me->m_hBackgroundBrush ? ::CreateSolidBrush(::GetSysColor(COLOR_BTNFACE)) : NppDarkMode::getDarkerBackgroundBrush();  // PapyrusPlugin modification -- only create brush if background is defined, otherwise use dark mode brush
        ::FillRect(hdc, &rect, hBrush);
        
        // PapyrusPlugin modification -- only delete brush if background is defined
        if (Me->m_hBackgroundBrush)
        {
          ::DeleteObject(hBrush);
        }
      ...
      switch (wParam)
      {
        ...
        default:
        {
            ...
            (Me->m_lpszModule[0] != _T('\0')))
          {
            ...
            WriteRegistry(Me->m_szCompanyName, szKey, static_cast<DWORD>(wParam));  // PapyrusPlugin modification -- cast from WPARAM to DWORD
          ...
          Me->m_nReturnValue = static_cast<int>(wParam);  // PapyrusPlugin modification -- cast from WPARAM to int
...
BOOL CXDialogTemplate::OnKeyDown(HWND hWnd, WPARAM wParam, LPARAM /*lParam*/)
{
  ...
  if (bCtrlIsDown && (ch == _T('C')))
  {
    ...
    if (::IsWindow(hWnd))
    {
      ...
      const TCHAR *pszDivider = _T("---------------------------\r\n");  // PapyrusPlugin modification -- add "const"
...
int CXDialogTemplate::Display()
{
  ...
  for (i = 0; i < m_dlgTempl.cdit; i++)
  {
    ...
    int nActualChars = static_cast<int>(_tcslen(m_pDlgItemArray[i]->m_pszCaption)) + 1;  // PapyrusPlugin modification -- cast from size_t to int
#else
  ...
  *(WORD*)pdest = 0;    // use default window class    //+++1.5
  ...
  for (i = 0; i < m_dlgTempl.cdit; i++)
  {
    ...
    *(WORD*)pdest = (WORD)m_pDlgItemArray[i]->m_controltype;  // atom value for the control
    ...
    pchCaption = new WCHAR[nChars+100];  // PapyrusPlugin modification -- remove redefinition

#ifdef _UNICODE
    memset(pchCaption, 0, nChars*sizeof(TCHAR) + 2);
    memcpy(pchCaption, m_pDlgItemArray[i]->m_pszCaption, nChars * sizeof(TCHAR));  //+++1.5
    nActualChars = nChars;  // PapyrusPlugin modification -- remove redefinition
#else
    nActualChars = MultiByteToWideChar(CP_ACP, 0,  // PapyrusPlugin modification -- remove redefinition
...
static DWORD ReadRegistry(LPCTSTR lpszCompanyName, LPCTSTR lpszKey)
{
  ...
  const TCHAR * szRegPath = _T("Software\\");  // PapyrusPlugin modification -- add "const"
